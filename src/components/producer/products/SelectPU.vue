<template>
	...
	<OverlayPanel ref="manageProductOverlay">
		<div v-if="isOverlayOpen" class="p-3">
			...
			{{ productionUnit.items.value.items }}

			<!-- TODO: Ver o que este emit faz -->
			<div class="flex justify-content-center mt-4">
				<VeeForm @submit="($e) => createProduct($e as Event)" class="flex flex-column gap-3">
					<!-- Dropdown -->
					...
					<!-- Submit -->
					<PrimeButton type="submit" :label="`${methodName}`" :loading="submitting" />
				</VeeForm>
			</div>
		</div>
	</OverlayPanel>

	<PrimeButton :icon="`pi pi-${methodName === 'Criar' ? 'plus' : 'truck'}`" outlined @click="toggleOverlay" />
</template>
...
export default {
	...
	methods: {
		fetchPUS() {
			this.contries = {};
			for (let i = 0; i < productionUnit.items.value.items.length; i++) {
				this.countris.push({ name: productionUnit.items.value[i].name, code: productionUnit.items.value[i].id });
			}
		},
	},
		...
		const manageProduct = handleSubmit(async (values) => {
			...
			try {
				...
				toast.add({
					severity: 'error',
					summary: 'Erro',
					detail: `Ocorreu um erro ao criar o produto, por favor tente novamente.<br/>${(error as Error).message
						}`,
					life: 10000,
				});
			}
