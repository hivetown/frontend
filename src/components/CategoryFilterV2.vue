    ...
    <div>
        <ul>
            <li v-for="category in getTopLevelCategories()" :key="category.id">
                ...
                <div v-if="category.subCategories" v-html="category.subCategoriesHtml"></div>
            </li>
        </ul>
    </div>
</template>
...
export default {
    ...
    methods: {
        async fetchSubCategories(category) {
            ...
            const subCategoriesResponse = await fetchCategorySubCategories(category.id);
        ...
        printSubCategories(subCategories) {
          ...
          <li key="${subCategory.id}">
            ...
            ${subCategory.subCategories
                        ? `<ul>${this.printSubCategories(subCategory.subCategories)}</ul>`
                        : ""
                    }
          </li>
        `;
