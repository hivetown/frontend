<template>
  ...
  >
    <b-card-text>
      ...
      <div v-if="qtd>0" v-for="idx in qtd" :key="idx">
        <p><strong>Nome: </strong>{{users['productionUnits'][idx-1]['name']}}, <strong>ID:</strong> {{users['productionUnits'][idx-1]['id']}}, <strong>AddressID:</strong> {{users['productionUnits'][idx-1]['address']}}</p>
      </div>
      <div v-else>
  ...
  import {getProducerId, desativarProducer, ativarProducer, updateProducer} from '../api/producers'
  ...
  export default {
    setup() {
      ...
      return { users, id, collapsed, formData, qtd };
    },
